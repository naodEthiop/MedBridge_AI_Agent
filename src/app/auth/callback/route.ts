...
export async function GET(request: Request) {
  ...
  if (!code) {
    return NextResponse.redirect(new URL("/auth/login?error=missing_oauth_code", PROD_SITE_URL));
  }
  ...
  try {
    ...
    if (error) {
      return NextResponse.redirect(new URL("/auth/login?error=auth_callback_failed", PROD_SITE_URL));
    }
    ...
    if (!onboardingComplete) return NextResponse.redirect(new URL("/onboarding", PROD_SITE_URL));
